...
class Icosahedron {
  ...
  init() {
    ...
    var point = new Point(
      new p5.Vector(0, -(this.triHt / 2.0 + this.a), 0),
      this
    );
...
class Point {
  constructor(position, parent = null) {
    ...
    if (this.parent)
      this.dPosition = p5.Vector.add(this.parent.position, this.pos_default);
  ...
  update() {
    if (this.isMoving) {
      ...
      if (this.parent) {
        ...
        if (effects[effectNum] == "wobble")
          copy.mult(
            sin(this.pos_default.x * 0.01 + frameCount * 0.08) * 0.15 + 1
          );
  ...
  display() {
    ...
    // if(effects[effectNum] == "connect"){
    //     var p = this.dPosition;
    //     for(let i=0;i<particles.length; i++){
    //         var tp = particles[i];
    //         pg.stroke(255);
    //         pg.strokeWeight(5);
    //         if(tp.dist(this.dPosition) < this.parent.radius*0.5){
    //             pg.line(p.x,p.y,p.z,tp.x,tp.y,tp.z);
    //         }
    //     }
    // }
  }
...
class Line {
  ...
  update() {}
  display() {
    if (this.visible && effects[effectNum] != "open") {
      pg.strokeWeight(10);
      pg.stroke(255 * lineRatio);
      pg.noFill();
      pg.beginShape();
...
class Face {
  ...
  display() {
    if (
      (this.visible &&
        effects[effectNum] != "connect" &&
        effects[effectNum] != "flicker") ||
      (this.visible && effects[effectNum] == "flicker" && frameCount % 8 <= 4)
    ) {
